...
def get_map_index(x,y):
    ...
    if row_idx % 2 == 1:
        ...
        if col_idx < 0:
            col_idx = 0
        elif col_idx > MAP_COLUM_COUNT - 2:
            col_idx = MAP_COLUM_COUNT - 2
...
def visit(row_idx, col_idx, color=None):
    # 맵의 범위를 벗어나는지 확인
    if row_idx < 0 or row_idx >= MAP_ROW_COUNT or col_idx < 0 or col_idx >= MAP_COLUM_COUNT:
        return
    # 현재 Cell의 색상이 color와 같은지 확인
    if color and map[row_idx][col_idx] != color:
        return
    #빈 공간이거나 버블이 존재 할 수 없는 위치인지 확인
...
def remove_hanging_bubbles():
    visited.clear()
    for col_idx in range(MAP_COLUM_COUNT):
        if map[0][col_idx] != ".":
            visit(0, col_idx)
...
#화살표 관련 변수
#to_angle = 0 #좌우로 움직일 각도
to_angle_left = 0 #왼쪽으로 움직일 각도 정보
to_angle_right = 0 
angle_speed = 1.5 #1.5도씩 움직인다
